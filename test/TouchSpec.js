...
define(["touch", "test/lib/mocked-jq", "test/lib/mocked-doc"], function (touch, mockedJQ, mockedDoc) {
    ...
    describe("Touch Command", function () {
        it("should touch element with ID myElement.", function () {
            ...
            //expect(touch.handle(mockedJQ.instance, "touch(\"#myElement\")"), { document: mockedDoc.instance }).toEqual({ handled: true, stop: false});
            //expect(mockedJQ.getCalls()).toEqual("find(#myElement);first();blur();trigger(touchstart);");
        });
        ...
        it("should fail when element is not found.", function () {
            ...
            //expect(touch.handle(mockedJQ.instance, "touch(\"#myElement\")", { document: mockedDoc.instance })).toEqual({ handled: false, stop: false});
