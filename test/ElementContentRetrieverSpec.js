...
define("ElementContentRetrieverSpec", ["elementContentRetriever", "test/lib/mocked-jq", "test/lib/mocked-doc"], function(retriever, mockedJQ, mockedDoc) {
  ...
  describe("Element Content Retriever", function () {
    ...
    it("should get content from an element and attribute (old style with quotes).", function () {
        ...
        expect(mockedJQ.getCalls()).toEqual("find(#myElement);first();attr(myAttribute);");
    });
    ...
    it("should get not content from an element attribute, and regex (new el function style).", function () {
        expect(retriever.getContent(mockedJQ.instance, "el(\"#myElement\", \"myAttribute\", /ASDFASFS/i)", { document: mockedDoc.instance })).toEqual({match: true, content: null});
        expect(mockedJQ.getCalls()).toEqual("find(#myElement);first();attr(myAttribute);");
    });
  });
});
